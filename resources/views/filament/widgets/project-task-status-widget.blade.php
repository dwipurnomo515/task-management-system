    ...
    <x-filament::card>
        <div class="p-4">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">Project Task Status</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($this->getPaginatedProjects() as $project)
                    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">{{ $project->name }}</h4>
                        <div class="space-y-2">
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-500">Total Tasks</span>
                                <span class="text-sm font-semibold">{{ $project->todo_count + $project->in_progress_count + $project->done_count }}</span>
                            </div>
                            <div class="space-y-1">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500">To Do</span>
                                    <span class="text-sm font-medium text-yellow-600">{{ $project->todo_count }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500">In Progress</span>
                                    <span class="text-sm font-medium text-blue-600">{{ $project->in_progress_count }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500">Done</span>
                                    <span class="text-sm font-medium text-green-600">{{ $project->done_count }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500">Overdue</span>
                                    <span class="text-sm font-medium text-red-600">{{ $project->overdue_count }}</span>
                                </div>
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-500">Upcoming (7 days)</span>
                                    <span class="text-sm font-medium text-orange-600">{{ $project->upcoming_count }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
